...
interface MediaSourceBarProps {
  currentUrl: string;
  provider: 'local' | 'youtube' | 'vimeo' | 'standalone' | 'drive';
  fps: number;
  dropFrame: boolean;
  startTc: string;
  onUpdateSource: (url: string, provider: 'local' | 'youtube' | 'vimeo' | 'standalone') => void;
...
export const MediaSourceBar: React.FC<MediaSourceBarProps> = ({
  ...
  return (
    <div className="bg-[#111724] border border-[#20293d] rounded-2xl p-3 shadow-xl flex flex-col md:flex-row items-center justify-between gap-3">
      {/* Left: Direct URL input & Local upload */}
      <form onSubmit={handleUrlSubmit} className="flex-1 w-full flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-20 py-2 rounded-xl bg-[#090d14] border border-[#232d44] text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500 transition"
          />
          <button
            type="submit"
        ...
        <button
          ...
          title="Run standalone timecode clock"
        >
      ...
      <div className="flex items-center gap-2 w-full md:w-auto justify-end border-t md:border-t-0 border-[#1e273b] pt-2 md:pt-0">
        {/* FPS selector */}

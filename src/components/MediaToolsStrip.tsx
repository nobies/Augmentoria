...
export const MediaToolsStrip: React.FC<MediaToolsStripProps> = ({
  ...
  return (
    <div className="flex lg:flex-col items-center justify-center gap-1.5 sm:gap-2 bg-[#111724] border border-[#20293d] p-1.5 sm:p-2 rounded-2xl shadow-xl shrink-0">
      {/* 1. Freeze Frame Drawing */}
      <button
        type="button"
        onClick={onStartDrawing}
        className={`p-2 sm:p-2.5 rounded-xl border transition active:scale-95 group relative ${
          hasActiveDrawing
        ...
        aria-label="Draw on Freeze Frame"
      >
      ...
      <button
        ...
        className="p-2 sm:p-2.5 rounded-xl bg-[#151b29] hover:bg-[#1a2336] border border-[#222c42] hover:border-orange-500/40 text-slate-300 hover:text-orange-400 transition active:scale-95"
      ...
      <label className="cursor-pointer">
        <div
          className="p-2 sm:p-2.5 rounded-xl bg-[#151b29] hover:bg-[#1a2336] border border-[#222c42] hover:border-blue-500/40 text-slate-300 hover:text-blue-400 transition active:scale-95"
          title="Place Watermark / Plate / Reference Image on Frame"
          aria-label="Upload reference image"
        >
          <ImageIcon className="w-4 h-4" />
        </div>
        <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" aria-label="Choose image file" />
      </label>
